...
export async function GET(req) {
  ...
  const inregistrari = await prisma.inregistrare.findMany({
    where: { registruId: Number(registruId) },
    include: { fisiere: true },
  })
  ...
  if (format === "csv") {
    ...
    // Pentru export Excel: doar coloanele vizibile în lista-inregistrari.jsx
    if (format === "excel" || !format) {
      ...
      inregistrari.forEach(row => {
        // Data Document: ia data primului fișier dacă există
        let dataFisier = '';
        ...
        // Destinatar: preferă destinatarUtilizator.nume, altfel destinatar (string)
        let destinatarNume = row.destinatarUtilizator?.nume || row.destinatar || '';
